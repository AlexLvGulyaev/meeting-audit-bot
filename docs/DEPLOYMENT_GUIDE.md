...
> 📌 Сессия админки ставится как `secure` cookie — она передаётся только по HTTPS. По plain HTTP (`http://localhost:8000`) health-эндпоинты и Telegram-бот работают, но вход в `/admin` зациклится на форме логина — проверку админки (RBAC, test-provider) выполняйте на инстансе с TLS.
